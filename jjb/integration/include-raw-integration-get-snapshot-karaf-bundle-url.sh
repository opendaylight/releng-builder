pwd
ls -lAn snapshot-karaf/snapshot-karaf-distribution/pom.xml
cat snapshot-karaf/snapshot-karaf-distribution/pom.xml

# Extract the BUNDLEVERSION from the pom.xml
BUNDLEVERSION=`xpath snapshot-karaf/snapshot-karaf-distribution/pom.xml '/project/version/text()' 2> /dev/null`
echo "Bundle version is ${BUNDLEVERSION}"

BUNDLEURL=${BUILD_URL}org.opendaylight.integration\$distribution-karaf/artifact/org.opendaylight.integration/snapshot-karaf-distribution/${BUNDLEVERSION}/snapshot-karaf-distribution-${BUNDLEVERSION}.zip
