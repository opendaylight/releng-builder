...
file_name=${WORKSPACE}/local.conf_control
cat > ${file_name} << EOF
[[local|localrc]]
...
enable_plugin networking-odl https://github.com/openstack/networking-odl  ${ODL_ML2_BRANCH}
...
HOSTIP=$1
file_name=${WORKSPACE}/local.conf_compute_${HOSTIP}
cat > ${file_name} << EOF
[[local|localrc]]
LOGFILE=stack.sh.log
LOG_COLOR=False
...
do
    OS_IP=OPENSTACK_SYSTEM_${i}_IP
    scp "${!OS_IP}:/opt/stack/log/stack.sh.log" "openstack_${i}.log"
done
