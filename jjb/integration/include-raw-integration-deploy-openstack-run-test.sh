...
do
    ${SSH} $ip "ovs-vsctl add-br $PUBLIC_BRIDGE"
    ${SSH} $ip "sudo ovs-vsctl set bridge $PUBLIC_BRIDGE other-config:disable-in-band=true"

    # Control node br-ex will act as the external router
    if [ "$ip" == "${OPENSTACK_CONTROL_NODE_IP}"]; then
        ${SSH} $ip "sudo ifconfig br-ex up 10.10.10.250/24"
    fi
    for remote_ip in ${DEVSTACK_IPS[*]}
    do
        if [ "$ip" != "$remote_ip" ]; then
            PORT_NAME=vxlan_${remote_ip}
