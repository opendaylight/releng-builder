...
echo "PATCHING NEWTON  GBP testplan :${TESTPLAN}  and ${ODL_ML2_BRANCH}"

if [ "${ODL_ML2_BRANCH}" == "stable/newton" ] && [[ "${TESTPLAN}" == *"xxgroupbasedpolicy-clustering"* ]]; then
echo "THIS IS NEWTON PATCH FOR GBP clustering tests"
cat >> ${local_conf_file_name} << EOF
# for NEWTON patch on  GBP
