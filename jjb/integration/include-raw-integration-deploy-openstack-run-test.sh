...
do
    ...
    create_control_node_local_conf ${!CONTROLIP}
...
do
    ...
    if [[ ${CONTROLLERFEATURES} == *federation* ]]; then
        scp ${WORKSPACE}/rabbit-server-script.sh ${!CONTROLIP}:/tmp
        ssh ${!CONTROLIP} "sudo bash /tmp/rabbit-server-script.sh"
    fi
...
compute_index=1
for compute_ip in ${compute_list[*]}
do
