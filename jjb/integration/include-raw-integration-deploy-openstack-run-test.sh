...
if [ "${ODL_ML2_BRANCH}" == "stable/mitaka" ]; then
ssh ${OPENSTACK_CONTROL_NODE_IP} "cd /opt/stack; git clone https://git.openstack.org/openstack/requirements; cd requirements; git checkout stable/mitaka; sed -i 's/openstacksdk/d' upper-constraints.txt; sed -i 's/libvirt-python/d' upper-constraints.txt"
...
do
    ...
    if [ "${ODL_ML2_BRANCH}" == "stable/mitaka" ]; then
       ssh ${!COMPUTEIP} "cd /opt/stack; git clone https://git.openstack.org/openstack/requirements; cd requirements; git checkout stable/mitaka; sed -i 's/libvirt-python/d' upper-constraints.txt"
    fi
