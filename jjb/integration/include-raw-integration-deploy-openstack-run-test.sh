...
PUBLIC_PHYSICAL_NETWORK=phys1 # FIXME this should be a parameter
ML2_VLAN_RANGES=phys1
...
physical_network_mtus = phys1:1440
...
${SSH} ${OPENSTACK_CONTROL_NODE_IP} "sudo ip link add link br-phys1 name br-phys1.167 type vlan id 167"
${SSH} ${OPENSTACK_CONTROL_NODE_IP} "sudo ifconfig"
${SSH} ${OPENSTACK_CONTROL_NODE_IP} "sudo ifconfig br-phys1 up"
${SSH} ${OPENSTACK_CONTROL_NODE_IP} "sudo ifconfig"
${SSH} ${OPENSTACK_CONTROL_NODE_IP} "sudo ifconfig  br-phys1.167 up ${GATEWAY_IP}/24"
${SSH} ${OPENSTACK_CONTROL_NODE_IP} "sudo ifconfig"
compute_index=1
