...
do
    ...
    ${SSH} $ip "sudo ovs-vsctl add-br $PUBLIC_BRIDGE -- set bridge $PUBLIC_BRIDGE other-config:disable-in-band=true other_config:hwaddr=f6:00:00:ff:01:00 stp_enable=true"
    ...
    do
        if [ "$ip" != "$remote_ip" ]; then
            ...
            ${SSH} $ip "sudo ovs-vsctl add-port $PUBLIC_BRIDGE $PORT_NAME -- set interface $PORT_NAME type=vxlan options:local_ip="$ip" options:remote_ip="$remote_ip" options:dst_port=9876 options:key=flow"
        fi
    done
done
