...
do
    if [ "${ENABLE_HAPROXY_FOR_NEUTRON}" == "yes" ]; then
        ...
        if [ $(( $i % (${NUM_OPENSTACK_SYSTEM} / ${NUM_OPENSTACK_SITES}) )) == 0 ]); then
            echo "Need to configure HAProxy"
            configure_haproxy_for_neutron_requests ${!ODLMGRIP}
            continue
        fi
    else
        ODLMGRIP=ODL_SYSTEM_$(( odlmgr_index++ ))_IP
    fi
    if [ $(( $i % (${NUM_OPENSTACK_SYSTEM} / ${NUM_OPENSTACK_SITES}) )) == 1 ]; then
        # FIXME sandra need to check
        ODLIP=ODL_SYSTEM_$(( odl_index++ ))_IP
        ODLOVSLIST=${!ODLIP}
        for j in `seq $odl_index ${NUM_ODL_SYSTEM}`
        do
            ODLIP=ODL_SYSTEM_${j}_IP
            ODLOVSLIST=${!ODLOVSLIST},${!ODLIP}
            if [$j % $(( ${NUM_ODL_SYSTEM} / ${NUM_OPENSTACK_SITES} )) == 0 ]; then
                echo ${ODLOVSLIST}
                odl_index=$(( j++ ))
                break
            fi
...
#FIXME sandra need to check code from here
control_index=1
for i in `seq 1 ${NUM_OPENSTACK_SYSTEM}`
do
    if [ $(( $i % (${NUM_OPENSTACK_SYSTEM} / ${NUM_OPENSTACK_SITES}) )) == 1 ]; then
