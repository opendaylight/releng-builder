...
scp ${OPENSTACK_CONTROL_NODE_IP}:/opt/stack/log/stack.sh.log "openstack_control_stack.log"
for i in `seq 1 $((NUM_OPENSTACK_SYSTEM - 1))
do
    OSIP=OPENSTACK_COMPUTE_NODE_${i}_IP
    scp "${!OSIP}:/opt/stack/log/stack.sh.log" "openstack_compute_stack_${i}.log"
done
