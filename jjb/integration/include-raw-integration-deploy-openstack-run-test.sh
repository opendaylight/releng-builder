...
# Openstacksdk,libvirt-python -> the current version does not work with  Mitaka diue to some requirements conflict and breaks when trying to  stack
# paramiko -> Problems with tempest tests due to paramiko incompatibility with pycrypto.  the problem has been solved with  version 1.17.  If the latest version  of
# paramiko  is used,  it causes other timeout problems 
