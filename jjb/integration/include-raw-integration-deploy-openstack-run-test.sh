...
do
    ...
    if [ $(( $i % (${NUM_OPENSTACK_SYSTEM} / ${NUM_OPENSTACK_SITES}) )) == 1 ]; then
        ...
        if [ "${ENABLE_HAPROXY_FOR_NEUTRON}" == "yes" ] && [ $(( $compute_index % (${NUM_OPENSTACK_SYSTEM} / ${NUM_OPENSTACK_SITES}) )) == 0 ]; then
            # skip ha proxy compute
...
-v ODL_SYSTEM_3_IP:${ODL_SYSTEM_3_IP} -v NUM_ODL_SYSTEM:$(( ${NUM_ODL_SYSTEM} / ${NUM_OPENSTACK_SITES} )) -v CONTROLLER_USER:${USER} -v OS_USER:${USER} \
-v NUM_OS_SYSTEM:$(( ${NUM_OPENSTACK_SYSTEM} / ${NUM_OPENSTACK_SITES} )) -v OS_CONTROL_NODE_IP:${OPENSTACK_CONTROL_NODE_IP} \
