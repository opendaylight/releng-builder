...
# shellchecker disable=SC2034 foo appears unused. Verify it or export it.
function configure_karaf_log() {
    ...
    if [[ "$KARAF_VERSION" == "karaf4" ]]; then
        ...
        ed -ie 's/log4j.appender.out.maxBackupIndex=10/log4j.appender.out.maxBackupIndex=1/g' ${LOGCONF}
        # FIXME: Make log size limit configurable from build parameter.
