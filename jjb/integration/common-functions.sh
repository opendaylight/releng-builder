...
def test_get_dirs_to_process(bash_arguments):
    global dirs_to_process
    # command get full path to directories
    cmd = 'ls -d \$PWD/src/main/yang/*/'
    response = pytest_lib.execute_ssh_command(command=cmd, username=bash_arguments['CONTROLLER_USER'], hostname=bash_arguments['ODL_SYSTEM_IP'])
    # convert bytest output to list and remove last item
    dirs_to_process = (response[0].decode("utf-8").split("\n"))[:-1]
    assert response[1] == 0
    
def test_get_yang_files_from_dirs(bash_arguments):
    global collected_yang_files
    collected_yang_files = []
    # print(dirs_to_process)
    for dir in dirs_to_process:
        cmd = f'find {dir} -name "*.yang"'
        response = pytest_lib.execute_ssh_command(command=cmd, username=bash_arguments['CONTROLLER_USER'], hostname=bash_arguments['ODL_SYSTEM_IP'])
        yang_files_in_dir = (response[0].decode("utf-8").split("\n"))[:-1]
        collected_yang_files.extend(yang_files_in_dir)
        # print(collected_yang_files)
        print(len(collected_yang_files))
    assert response[1] == 0
EOF
}
