...
FEATURESCONF="${WORKSPACE}/${BUNDLE_FOLDER}/etc/org.apache.karaf.features.cfg"
...
LOGCONF="${WORKSPACE}/${BUNDLE_FOLDER}/etc/org.ops4j.pax.logging.cfg"
sed -ie 's/log4j.appender.out.maxFileSize=1MB/log4j.appender.out.maxFileSize=20MB/g' "${LOGCONF}"
cat "${LOGCONF}"

echo "Configure max memory..."
MEMCONF="${WORKSPACE}/${BUNDLE_FOLDER}/bin/setenv"
...
export KARAF_REDIRECT="${WORKSPACE}/${BUNDLE_FOLDER}/data/log/karaf_console.log"
mkdir -p ${WORKSPACE}/${BUNDLE_FOLDER}/data/log

echo "Starting controller..."
${WORKSPACE}/${BUNDLE_FOLDER}/bin/start

function dump_log_and_exit {
    echo "Dumping first 500K bytes of karaf log..."
    head --bytes=500K "${WORKSPACE}/${BUNDLE_FOLDER}/data/log/karaf.log"
    echo "Dumping last 500K bytes of karaf log..."
    tail --bytes=500K "${WORKSPACE}/${BUNDLE_FOLDER}/data/log/karaf.log"
    cp "${WORKSPACE}/${BUNDLE_FOLDER}/data/log/karaf.log" .
    cp "${WORKSPACE}/${BUNDLE_FOLDER}/data/log/karaf_console.log" .
    exit 1
}
...
if [ "${DISTROSTREAM}" == "carbon" ] || [ "${DISTROSTREAM}" == "nitrogen" ]; then
    ...
    while true; do
        ...
        sleep 5
        echo "already waited ${COUNT} seconds..."
        if grep --quiet 'org.opendaylight.infrautils.ready-impl.*System ready' "${WORKSPACE}/${BUNDLE_FOLDER}/data/log/karaf.log"; then
            echo "Controller is UP"
            break
        elif (( "${COUNT}" > "300" )); then
            echo "Timeout Controller DOWN"
...
# sshpass -p karaf ${WORKSPACE}/${BUNDLE_FOLDER}/bin/client -u karaf 'bundle:list'
...
function exit_on_log_file_message {
    ...
    if grep --quiet "$1" "${WORKSPACE}/${BUNDLE_FOLDER}/data/log/karaf.log"; then
        echo ABORTING: found "$1"
        dump_log_and_exit
    fi

    echo "looking for \"$1\" in karaf_console.log file"
    if grep --quiet "$1" "${WORKSPACE}/${BUNDLE_FOLDER}/data/log/karaf_console.log"; then
        echo ABORTING: found "$1"
        dump_log_and_exit
    fi
}
...
pushd "${WORKSPACE}/${BUNDLE_FOLDER}" || exit
XMLS_FOUND="$(echo *.xml)"
popd || exit
