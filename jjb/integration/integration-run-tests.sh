...
ssh ${OPENSTACK_CONTROL_NODE_1_IP} "sudo ovs-vsctl --if-exists del-port br-int br-datacentre"
ssh ${OPENSTACK_CONTROL_NODE_1_IP} "sudo ovs-vsctl --may-exist add-br br-datacentre -- set bridge br-datacentre other-config:disable-in-band=true other_config:hwaddr=f6:00:00:ff:01:01"

ssh ${OPENSTACK_COMPUTE_NODE_1_IP} "sudo ovs-vsctl --if-exists del-port br-int br-datacentre"
ssh ${OPENSTACK_COMPUTE_NODE_1_IP} "sudo ovs-vsctl --may-exist add-br br-datacentre -- set bridge br-datacentre other-config:disable-in-band=true other_config:hwaddr=f6:00:00:ff:01:02"

ssh ${OPENSTACK_COMPUTE_NODE_2_IP} "sudo ovs-vsctl --if-exists del-port br-int br-datacentre"
ssh ${OPENSTACK_COMPUTE_NODE_2_IP} "sudo ovs-vsctl --may-exist add-br br-datacentre -- set bridge br-datacentre other-config:disable-in-band=true other_config:hwaddr=f6:00:00:ff:01:03"

ssh ${OPENSTACK_CONTROL_NODE_1_IP} "
        sudo ovs-vsctl add-port br-datacentre compute_1_vxlan -- set interface compute_1_vxlan type=vxlan options:local_ip=${OPENSTACK_CONTROL_NODE_1_IP} options:remote_ip=${OPENSTACK_COMPUTE_NODE_1_IP} options:dst_port=9876 options:key=flow
"
ssh ${OPENSTACK_CONTROL_NODE_1_IP} "
    sudo ovs-vsctl add-port br-datacentre compute_2_vxlan -- set interface compute_2_vxlan type=vxlan options:local_ip=${OPENSTACK_CONTROL_NODE_1_IP} options:remote_ip=${OPENSTACK_COMPUTE_NODE_2_IP} options:dst_port=9876 options:key=flow
"
ssh ${OPENSTACK_COMPUTE_NODE_1_IP} "
        sudo ovs-vsctl add-port br-datacentre control_1_vxlan -- set interface control_1_vxlan type=vxlan options:local_ip=${OPENSTACK_COMPUTE_NODE_1_IP} options:remote_ip=${OPENSTACK_CONTROL_NODE_1_IP} options:dst_port=9876 options:key=flow
"
ssh ${OPENSTACK_COMPUTE_NODE_2_IP} "
        sudo ovs-vsctl add-port br-datacentre control_1_vxlan -- set interface control_1_vxlan type=vxlan options:local_ip=${OPENSTACK_COMPUTE_NODE_2_IP} options:remote_ip=${OPENSTACK_CONTROL_NODE_1_IP} options:dst_port=9876 options:key=flow
"

ssh ${OPENSTACK_CONTROL_NODE_1_IP} "sudo iptables --line-numbers -nvL"
ssh ${OPENSTACK_COMPUTE_NODE_1_IP} "sudo iptables --line-numbers -nvL"
ssh ${OPENSTACK_COMPUTE_NODE_2_IP} "sudo iptables --line-numbers -nvL"
