...
if [ ${BUNDLEURL} == 'last' ]; then
    ...
    BUNDLEVERSION=`xpath pom.xml '(//version)[1]/text()' 2> /dev/null`
...
pybot -N ${TESTPLAN} -c critical -v CONTROLLER:${CONTROLLER0} -v MININET:${MININET0} -v MININET_USER:${USER} -v USER_HOME:${HOME} ${TESTOPTIONS} ${SUITES}
