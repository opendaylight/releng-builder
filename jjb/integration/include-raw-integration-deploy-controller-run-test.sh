...
SCRIPTPLAN="${TESTPLAN%.*}-script.txt"

if [ -f ${WORKSPACE}/test/csit/testplans/${SCRIPTPLAN} ]; then
    echo "scriptplan exists!!!"
    echo "Changing the scriptplan path..."
    cat ${WORKSPACE}/test/csit/testplans/${SCRIPTPLAN} | sed "s:integration:${WORKSPACE}:" > scriptplan.txt
    cat scriptplan.txt
