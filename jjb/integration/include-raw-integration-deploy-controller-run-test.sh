...
scp ${WORKSPACE}/configuration-script.sh ${ODL_SYSTEM_IP}:${WORKSPACE}
ssh ${ODL_SYSTEM_IP} 'bash ${WORKSPACE}/configuration-script.sh'
...
scp ${WORKSPACE}/startup-script.sh ${ODL_SYSTEM_IP}:${WORKSPACE}
ssh ${ODL_SYSTEM_IP} 'bash ${WORKSPACE}/startup-script.sh'
...
if [ "x$KARAF_PID" != "x" ]; then
    ssh ${ODL_SYSTEM_IP} "jstack $KARAF_PID"> ${WORKSPACE}/karaf_threads_before.log
else
    echo "No running Karaf found! OutOfMemory happened?"
fi
...
pybot -N ${TESTPLAN} -c critical -e exclude -v BUNDLEFOLDER:${BUNDLEFOLDER} -v WORKSPACE:${WORKSPACE} \
...
ssh ${ODL_SYSTEM_IP} gzip --best "${WORKSPACE}/${BUNDLEFOLDER}/data/log/karaf.log"
echo "Fetching compressed karaf.log"
scp "${ODL_SYSTEM_IP}:${WORKSPACE}/${BUNDLEFOLDER}/data/log/karaf.log.gz" .
# TODO: Should we compress the output log file as well?
scp "${ODL_SYSTEM_IP}:${WORKSPACE}/${BUNDLEFOLDER}/data/log/karaf_console.log" .
