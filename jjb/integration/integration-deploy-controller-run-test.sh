...
if [ -f ${WORKSPACE}/csit/scriptplans/${TESTPLAN} ]; then
    ...
    cat ${WORKSPACE}/csit/scriptplans/${TESTPLAN} | sed "s:integration/test:${WORKSPACE}:" > scriptplan.txt
    cat scriptplan.txt
...
cat ${WORKSPACE}/csit/testplans/${TESTPLAN} | sed "s:integration/test:${WORKSPACE}:" > testplan.txt
cat testplan.txt
