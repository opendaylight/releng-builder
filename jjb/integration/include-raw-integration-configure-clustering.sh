...
# Create car-people shards for controller cluster test
if [[ \$2 == *"controller-csit-3node-clustering"* ]]; then
echo "Add car-people shards"
cat >> /tmp/${BUNDLEFOLDER}/bin/custom_shard_config.txt <<EndOfFile
...
do
    ...
    ssh ${!CONTROLLERIP} "bash /tmp/configuration-script.sh ${i} ${JOB_NAME}"
done
