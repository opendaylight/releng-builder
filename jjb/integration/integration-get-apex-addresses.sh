...
openstack object save OPNFV-APEX-SNAPSHOTS ~/.ssh/id_rsa

NUM_OPENSTACK_SYSTEMS=3
ODL_SYSTEM_IP=$(python releng/jjb/cperf/parse-node-yaml.py get_value -k address --node-type controller node.yaml)
OPENSTACK_COMPUTE_NODE_1_IP=$(python releng/jjb/cperf/parse-node-yaml.py get_value -k address --node-type compute --node-number 1 node.yaml)
OPENSTACK_COMPUTE_NODE_2_IP=$(python releng/jjb/cperf/parse-node-yaml.py get_value -k address --node-type compute --node-number 2 node.yaml)
