...
# FIXME: Currently, swapping out the ODL distribution is hitting some bugs and
#        does not work. Once those are resolved, we can uncomment the below lines
...
sudo /usr/sbin/ip link set dev eth0 mtu 1458
sudo /usr/sbin/ip link set dev br-int mtu 1458
sudo /usr/sbin/ip link set dev br-ex mtu 1458
sudo /usr/sbin/ip link set dev ovs-system mtu 1458
sudo setenforce 0
...
ssh $OPENSTACK_CONTROL_NODE_1_IP "sudo docker restart neutron_api;"
...
ssh $OPENSTACK_COMPUTE_NODE_1_IP "sudo docker restart nova_compute nova_libvirt"
...
ssh $OPENSTACK_COMPUTE_NODE_2_IP "sudo docker restart nova_compute nova_libvirt"
