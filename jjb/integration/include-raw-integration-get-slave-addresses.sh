...
if [ "${ENABLE_HAPROXY_FOR_NEUTRON}" == "yes" ]; then
   echo "NUM_OPENSTACK_SYSTEM=$(( ${#OPENSTACK_SYSTEM[@]} - ${NUM_OPENSTACK_CONTROL_NODE} ))" >> slave_addresses.txt
else
...
for i in `seq 0 $(( ${NUM_OPENSTACK_CONTROL_NODE} - 1 ))`
do
    echo "OPENSTACK_CONTROL_NODE_$((i+1))_IP=${OPENSTACK_SYSTEM[${i}]}" >> slave_addresses.txt
done

for i in `seq 0 $(( ${#OPENSTACK_SYSTEM[@]} - ${NUM_OPENSTACK_CONTROL_NODE} - 1 ))`
do
    echo "OPENSTACK_COMPUTE_NODE_$((i+1))_IP=${OPENSTACK_SYSTEM[${i+$NUM_OPENSTACK_CONTROL_NODE}]}" >> slave_addresses.txt
done
