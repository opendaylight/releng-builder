...
if [[ "${PATCHES_TO_BUILD}" == *topic* ]]; then
    ...
    PATCHES_TO_BUILD=""
    PROJECT_LIST=(${BUILD_ORDER})
    for PROJECT in ${PROJECT_LIST[@]}; do
        # get all patches number for a topic for a given project
        GERRIT_PATCH_LIST=(`ssh -p 29418 "jenkins-$SILO@git.opendaylight.org" gerrit query status:open topic:${TOPIC} project:${PROJECT} \
        ...
        if [[ ! -z "${GERRIT_PATCH_LIST[@]}" ]]; then
            ...
            IFS=$'\n' SORT_REF_LIST=($(sort <<<"${REF_LIST[*]}")) && unset IFS
