...
EOF
    ...
    if ${SSH} ${OPENSTACK_CONTROL_NODE_1_IP} "sudo sh -c '[ -f ${DEVSTACK_TEMPEST_DIR}/${TESTREPO}/0 ]'"; then
        ...
        if [[ "$(echo ${OPENSTACK_BRANCH} | cut -d/ -f2)" == "stable/ocata" || "$(echo ${OPENSTACK_BRANCH} | cut -d/ -f2)" == "stable/pike" ]]; then
...
for i in `seq 1 ${NUM_OPENSTACK_CONTROL_NODES}`; do
    ...
    if [[ "${ODL_ML2_BRANCH}" == "stable/queens" || "${ODL_ML2_BRANCH}" == "master" ]]; then
