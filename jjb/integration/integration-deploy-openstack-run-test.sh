...
function collect_openstack_logs() {
    ...
    rsync -avhe ssh odl@${ip}:/opt/stack/logs/* ${oslogs} # rsync to prevent copying of symbolic links

    # Starting with queens break out the logs from journalctl
    if [ "${OS_BRANCH}" = "stable/queens" ]; then
...
EOF
        scp ${WORKSPACE}/collect_openstack_logs.sh odl@${ip}:/tmp
        ${SSH} odl@${ip} "bash /tmp/collect_openstack_logs.sh"
        rsync -avhe ssh odl@${ip}:/tmp/oslogs/ ${oslogs}
    fi
}
