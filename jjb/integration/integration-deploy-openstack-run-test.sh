...
if [ "${OVS_INSTALL:0:1}" = "v" ]; then
    ...
    [ ! -d "${rpm_path}" ] 
        && mkdir -p "${rpm_path}" 
	&& build_ovs ${node} ${OVS_INSTALL}
	&& scp ${node}:/tmp/ovs_rpms/*.rpm "${rpm_path}/"
    # Install OVS from path
    install_ovs_from_path "${rpm_path}"
else
...
for i in `seq 1 ${NUM_OPENSTACK_CONTROL_NODES}`; do
    ...
    [ -n "${OVS_INSTALL}" ] && install_ovs ${!CONTROLIP} /tmp/ovs_rpms/control
...
for i in `seq 1 ${NUM_OPENSTACK_COMPUTE_NODES}`; do
    ...
    [ -n "${OVS_INSTALL}" ] && install_ovs ${!COMPUTEIP} /tmp/ovs_rpms/compute
