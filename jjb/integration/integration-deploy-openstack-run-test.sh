...
# Checks is substring is within string
function contains() {
    string=$1
    substring=$2
...
# Checks is there is a control+compute node by looking for the n-cpu service
...
EOF
    ...
    find . > ${WORKSPACE}/archives/find.tmp1.txt
    ...
    for i in `seq 1 ${NUM_ODL_SYSTEM}`; do
        ...
        find . > ${WORKSPACE}/archives/find.tmp2.txt
