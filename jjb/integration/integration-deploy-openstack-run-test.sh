...
if [[ "${ENABLE_OS_PLUGINS}" =~ networking-sfc ]]; then
    wget https://cloud.centos.org/centos/6.6/images/CentOS-6-x86_64-GenericCloud.qcow2.xz -O /tmp/centos6.qcow2.xz
    unxz /tmp/centos6.qcow2.xz
    openstack image create centos6 --file /tmp/centos6.qcow2 --disk-format qcow2 --container-format bare --public
    ssh-keygen -f /tmp/odlkey -q -N ""
    openstack keypair create odltestkey --public-key /tmp/odlkey.pub
    scp /tmp/odlkey ${!CONTROLIP}:/tmp
    wget https://git.opendaylight.org/gerrit/gitweb?p=sfc.git;a=blob_plain;f=sfc-test/nsh-tools/vxlan_tool.py;hb=9a16198bd5de529a30cfe4a086fea6d3332667b4 -O /tmp/vxlan_tool.py
    scp /tmp/vxlan_tool.py ${!CONTROLIP}:/tmp
fi
