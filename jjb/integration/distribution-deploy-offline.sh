...
COUNT=0
while true; do
    RC=$(sshpass -p karaf ssh -p 8101 -tt karaf@127.0.0.1 "log:log Pinging karaf console" || echo $?)
    if [[ "x$RC" == "x" ]]; then
        echo Karaf is UP
        break
    elif (( "${COUNT}" > 600 )); then
...
sshpass -p karaf ssh -p 8101 -tt karaf@127.0.0.1 "feature:install ${ACTUALFEATURES}" || echo $? > "${WORKSPACE}/error.txt"
