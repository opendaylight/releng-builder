...
if [ "$BRANCH_CUT" = "true" ] && [ "$BRANCH" != "master" ]; then
    echo "Cannot branch cut on stable/"$BRANCH""
    exit 1
fi

if [ "$BRANCH_CUT" = "true" ]; then
    git checkout master
    git clean -ffdx
...
#git remote add gerrit ssh://jenkins-releng@devvexx.opendaylight.org:29418/mirror/releng/autorelease
git remote -v
git review -s
