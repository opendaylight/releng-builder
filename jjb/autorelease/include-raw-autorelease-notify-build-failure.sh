...
for p in $(find . -name pom.xml); do
    GROUP=$(xmlstarlet sel\
          ...
          -t -v "/x:project[x:artifactId='$ARTIFACT_ID']/x:groupId"\
          "$p" 2>/dev/null)
    if [ -z "${GROUP}" ]; then
	      GROUP=$(xmlstarlet sel\
              -N x=http://maven.apache.org/POM/4.0.0\
              -t -v "/x:project[x:artifactId='$ARTIFACT_ID']/x:parent/x:groupId"\
              "$p" 2>/dev/null)
    fi
    [ ! -z "${GROUP}" ] && grouplist+=($(echo "${GROUP}" | awk -F'.' '{ print $3 }'))
done

if [ "${#grouplist[@]}" -eq 1 ]; then
    PROJECT="${grouplist[0]}"
else
    GROUPLIST="NOTE: artifactId:$ARTIFACT_ID found under projects: ${grouplist[*]}."
fi
...
if [ ! -z "${ARTIFACT_ID}" ] && [[ "${BUILD_STATUS}" != "SUCCESS" ]]; then
    ...
    sed -e "/\[INFO\] Building \(${ARTIFACT_ID} \|ODL :: ${PROJECT} :: ${ARTIFACT_ID} \)/,/Reactor Summary:/!d;//d" \
