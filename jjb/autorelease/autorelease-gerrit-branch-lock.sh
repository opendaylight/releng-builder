...
if [ "${SUPERCOMMITTERS}" = true ]; then
    mode=supercommitters
elif [ "${CODE_FREEZE}" = true ]; then
    mode=code-freeze
elif [ "${RELEASE_PREP}" = true ]; then
    mode=release-prep
...
case $mode in
    supercommitters)
        echo "Locking branch for MRI: ${GERRIT_BRANCH}"
        ...
        ;;
    code-freeze)
        echo "Locking branch for code-freeze: ${GERRIT_BRANCH}"
        git config -f project.config "access.refs/heads/${GERRIT_BRANCH}.exclusiveGroupPermissions" "submit"
