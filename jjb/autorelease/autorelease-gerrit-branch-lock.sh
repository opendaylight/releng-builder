...
if [ "${SUPERCOMMITTERS}" = true ] && [ "${CODE_FREEZE}" = false ] && [ "${RELEASE_PREP}" = false ] && [ "${UNLOCK}" = false ] && [ "${DRY_RUN}" = false ]; then
    mode=supercommitters
elif [ "${CODE_FREEZE}" = true ] && [ "${SUPERCOMMITTERS}" = false ] && [ "${RELEASE_PREP}" = false ] && [ "${UNLOCK}" = false ] && [ "${DRY_RUN}" = false ]; then
    mode=code-freeze
elif [ "${RELEASE_PREP}" = true ] && [ "${SUPERCOMMITTERS}" = false ] && [ "${CODE_FREEZE}" = false ] && [ "${UNLOCK}" = false ] && [ "${DRY_RUN}" = false ]; then
    mode=release-prep
elif [ "${UNLOCK}" = true ] && [ "${SUPERCOMMITTERS}" = false ] && [ "${RELEASE_PREP}" = false ] && [ "${CODE_FREEZE}" = false ] && [ "${DRY_RUN}" = false ]; then
    mode=unlock
elif [ "${DRY_RUN}" = true ] && [ "${UNLOCK}" = false ] && [ "${SUPERCOMMITTERS}" = false ] && [ "${RELEASE_PREP}" = false ] && [ "${CODE_FREEZE}" = false ]; then
    mode=dry-run
else
    mode=none
fi
...
case $mode in
    ...
    dry-run)
        ...
        ;;
    *)
        echo "ERROR: Unknown options were selected:'$mode'."
        echo "ERROR: Select ONLY a single checkbox (SUPERCOMMITTERS|CODE_FREEZE|RELEASE_PREP|UNLOCK|DRY_RUN)"
        echo "ERROR: :'$mode'."
        exit 1
        ;;
esac
