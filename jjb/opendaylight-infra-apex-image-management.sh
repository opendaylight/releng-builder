...
qemu-img convert -f qcow2 -O raw /tmp/apex_snapshots/$CONTROLLER_NODE.qcow2 $CONTROLLER_NODE.raw
openstack image create \
  --disk-format raw --container-format bare \
  --file /tmp/apex_snapshots/$CONTROLLER_NODE.raw "OPNFV - apex - controller - 0"

qemu-img convert -f qcow2 -O raw /tmp/apex_snapshots/$COMPUTE_0_NODE.qcow2 $COMPUTE_0_NODE.raw
openstack image create \
  --disk-format raw --container-format bare \
  --file /tmp/apex_snapshots/$COMPUTE_0_NODE.raw "OPNFV - apex - compute - 0"

qemu-img convert -f qcow2 -O raw /tmp/apex_snapshots/$COMPUTE_1_NODE.qcow2 $COMPUTE_1_NODE.raw
openstack image create \
  --disk-format raw --container-format bare \
