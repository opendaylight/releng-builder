...
qemu-img convert -f qcow2 -O raw /tmp/apex_snapshots/$CONTROLLER_NODE.raw $CONTROLLER_NODE.raw
openstack image create \
  --disk-format raw --container-format bare \
  --file /tmp/apex_snapshots/$CONTROLLER_NODE.raw "OPNFV - apex - controller - 0"

qemu-img convert -f qcow2 -O raw /tmp/apex_snapshots/$COMPUTE_0_NODE.raw $COMPUTE_0_NODE.raw
openstack image create \
  --disk-format qcow2 --container-format raw \
  --file /tmp/apex_snapshots/$COMPUTE_0_NODE.raw "OPNFV - apex - compute - 0"

qemu-img convert -f qcow2 -O raw /tmp/apex_snapshots/$COMPUTE_1_NODE.raw $COMPUTE_1_NODE.raw
openstack image create \
  --disk-format qcow2 --container-format raw \
