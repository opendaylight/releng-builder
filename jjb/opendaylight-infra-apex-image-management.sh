...
for image in $images; do
  ...
  tar --delete snapshots.tar $image
done

# get the ssh keys and node.yaml and upload to swift
tar -xf snapshots.tar ./id_rsa
tar -xf snapshots.tar ./node.yaml

openstack container create OPNFV-APEX-SNAPSHOTS
openstack object create OPNFV-APEX-SNAPSHOTS node.yaml
openstack object create OPNFV-APEX-SNAPSHOTS id_rsa
...
openstack image delete $CONTROLLER_IMAGE_NAME
openstack image delete $COMPUTE_0_IMAGE_NAME
openstack image delete $COMPUTE_1_IMAGE_NAME
