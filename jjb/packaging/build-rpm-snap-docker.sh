...
sudo docker exec $docker_id /usr/bin/yum -n install curl expect nmap sudo openssh python-virtualenv python rpmdevtools rpmbuild git
...
sudo docker exec $docker_id git fetch https://git.opendaylight.org/gerrit/releng/builder refs/changes/61/68861/2 && git checkout FETCH_HEAD
