...
sudo docker exec $docker_id /usr/bin/yum -y install curl expect nmap sudo openssh python-virtualenv python rpmdevtools rpmbuild git
...
sudo docker exec $docker_id git --git-dir /builder/.git fetch https://git.opendaylight.org/gerrit/releng/builder refs/changes/61/68861/4
sudo docker exec $docker_id git --git-dir /builder/.git checkout FETCH_HEAD
