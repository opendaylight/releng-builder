...
docker_id=$(sudo docker run --env-file env-file -di --privileged -v /sys/fs/cgroup:/sys/fs/cgroup:ro -v $WORKSPACE/builder/jjb/packaging:/home/scripts opensuse /usr/lib/systemd/systemd)
...
# These lines are just added to try the patch in CI
