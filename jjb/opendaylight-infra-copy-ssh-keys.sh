...
if [ -z "$JCLOUDS_IPS" ]; then
    ...
    CONTROLLER_IPS=`openstack --os-cloud rackspace stack show -f json -c outputs $STACK_NAME | jq -r '.outputs[] | select(.output_key=="controller_ips") | .output_value[]'`
    MININET_IPS=`openstack --os-cloud rackspace stack show -f json -c outputs $STACK_NAME | jq -r '.outputs[] | select(.output_key=="mininet_ips") | .output_value[]'`
