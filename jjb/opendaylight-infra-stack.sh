...
openstack --os-cloud ${CLOUD_NAME} limits show --absolute
openstack --os-cloud ${CLOUD_NAME} limits show --rate
echo "Trying up to $STACK_RETRIES times to create $STACK_NAME."
for try in `seq $STACK_RETRIES`; do
    openstack --os-cloud ${CLOUD_NAME} stack create --timeout $OS_TIMEOUT -t {stack-template} -e $WORKSPACE/opendaylight-infra-environment.yaml --parameter "job_name=$VM_NAME" --parameter "silo=$SILO" $STACK_NAME
    openstack --os-cloud ${CLOUD_NAME} stack list
    echo "Waiting for $OS_TIMEOUT minutes to create $STACK_NAME."
    for i in `seq $OS_TIMEOUT`; do
        sleep 60
        OS_STATUS=`openstack --os-cloud ${CLOUD_NAME} stack show -f json -c stack_status $STACK_NAME | jq -r '.stack_status'`
        ...
        case "$OS_STATUS" in
            ...
            CREATE_FAILED)
                ...
                openstack --os-cloud ${CLOUD_NAME} stack list
                openstack --os-cloud ${CLOUD_NAME} stack delete --yes $STACK_NAME
                openstack --os-cloud ${CLOUD_NAME} stack show $STACK_NAME
                ...
                for i in `seq 20`; do
                    sleep 30;
                    STACK_SHOW=$(openstack --os-cloud ${CLOUD_NAME} stack show $STACK_NAME)
                    echo $STACK_SHOW
                    if [[ $STACK_SHOW == *"DELETE_FAILED"* ]]; then
                        echo "stack delete failed. trying to stack abandon now"
                        openstack --os-cloud ${CLOUD_NAME} stack abandon $STACK_NAME
                        STACK_SHOW=$(openstack --os-cloud ${CLOUD_NAME} stack show $STACK_NAME)
                        echo $STACK_SHOW
                    fi
