...
def update_integration_csit_list(
    ...
    """Updates the file based CSIT lists in integration-test-jobs.yaml."""
    ...
    def default_constructor(constructor, tag_suffix, node):
        generic = {ruamel.yaml.ScalarNode: GenericScalar,}.get(type(node))
        if generic is None:
    ...
    with open(defaults_yaml, "r") as f:
        ...
        new_csit_mri_list = copy.deepcopy(
            data[0]["defaults"].get("csit-weekly-list-{}".format(release_on_stable_branch))
        )
        data[0]["defaults"][
        ...
        new_csit_mri_list = copy.deepcopy(
            data[0]["defaults"].get("csit-sanity-list-{}".format(release_on_stable_branch))
        )
        data[0]["defaults"][
