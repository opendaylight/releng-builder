...
try:
    ...
    print("If ruamel.yaml is not available in your system's package manager you"
...
def update_integration_csit_list(
    ...
    with fileinput.FileInput(dest, inplace=True) as file:
        for line in file:
            print(
                line.replace(release_on_stable_branch, release_on_current_branch), end=""
            )
...
parser.add_argument(
    ...
    help="""The ODL release codename for the new {}
        (eg. Magnesium, Aluminium, Silicon).""".format(default_branch),
)
parser.add_argument(
    "job_dir",
    metavar="JOB_DIR",
