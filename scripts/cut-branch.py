        ...
        try:
            with open(
                ...
                if not data[0]["project"]["name"] == "{}-{}".format(
                    directory, release_on_stable_branch.lower()
                ):
                    continue
                ...
                new_job["project"]["name"] = "{}-{}".format(
                    directory, release_on_master_branch.lower()
                )
                new_job["project"]["branch"] = "master"
                new_job["project"]["stream"] = "{}".format(
                    release_on_master_branch.lower()
                )

                # Update exiting job for the new stable branch
                data[0]["project"]["branch"] = "stable/{}".format(
                    release_on_stable_branch.lower()
                )
        ...
        for job_file in filter(
            ...
            with open(os.path.join(job_dir, directory, job_file), "r") as f:
                ...
                for project in data:
                    ...
                    new_stream = {}
                    new_stream[release_on_master_branch.lower()] = copy.deepcopy(
                        streams[0].get(release_on_stable_branch.lower())
                    )

                    # Update the previous release stream branch to
                    # stable/{stream} instead of master
                    streams[0][release_on_stable_branch.lower()][
                        "branch"
                    ] = "stable/{}".format(release_on_stable_branch.lower())
...
create_and_update_project_jobs(
    args.release_on_stable_branch, args.release_on_master_branch, args.job_dir
)
update_job_streams(
    args.release_on_stable_branch, args.release_on_master_branch, args.job_dir
)
