...
def update_job_streams(release_on_stable_branch, release_on_master_branch, job_dir):
    ...
    for directory in filter(lambda d: os.path.isdir(os.path.join(job_dir, d)), os.listdir(job_dir)):
        for job_file in filter(lambda f: os.path.isfile(os.path.join(job_dir, directory, f)),
            ...
            with open(os.path.join(job_dir, directory, job_file), 'r') as f:
                ...
                for project in data:
                    ...
                    # Skip if the stream does not match release_on_stable_branch in the first item
                    if not streams[0].get(release_on_stable_branch.lower(), None):
                        continue

                    # Create the next release stream
                    new_stream = {}
                    new_stream[release_on_master_branch.lower()] = copy.deepcopy(streams[0].get(release_on_stable_branch.lower()))

                    # Update the previous release stream branch to stable/{stream} instead of master
                    streams[0][release_on_stable_branch.lower()]['branch'] = "stable/{}".format(release_on_stable_branch.lower())
...
update_job_streams(args.release_on_stable_branch, args.release_on_master_branch, args.job_dir)
