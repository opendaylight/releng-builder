...
cat > /opt/openstack/install/create_domain_service.sh << EOF
CONTROL_HOSTNAME=\$1
...
cat > /opt/openstack/install/setup_nova_core.sh << EOF
CONTROL_HOSTNAME=\$1
...
cat > /opt/openstack/install/setup_neutron_db.sh << EOF
CONTROL_HOSTNAME=\$1
...
cat > /opt/openstack/install/setup_neutron_server.sh << EOF
CONTROL_HOSTNAME=\$1
...
cat > /opt/openstack/install/setup_nova_compute.sh << EOF
CONTROL_HOSTNAME=\$1
MY_IP=\$2
...
cat > /opt/openstack/install/load_images_flavors.sh << EOF
source /tmp/client_rc
