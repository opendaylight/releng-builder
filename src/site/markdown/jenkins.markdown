...
### <a name="jjb_install_docker">Using Docker</a>
...
### <a name="jjb_install_manual">Manual Install</a>
...
dependencies. The tools [Virtualenv][18] and [Virtualenvwrapper][30] can help
you do so.

There are good docs for [installing Virtualenv][19] and [installing
Virtualenvwrapper][31]. On Linux systems with pip (typical), they amount to:

    sudo pip install virtualenv virtualenvwrapper

If you're just using Virtualenv, pick some location on your filesystem to
store/manage your virtual environments. You can then create a basic venv for
JJB (install pip and wheel, build correct dir structure), activate it (modify
your path), install [JJB's dependencies][20] and install JJB.

    # From <your venv managment dir>
    # Create basic venv (intall pip, wheel, dir structures)
    $ virtualenv jjb
    # Path before activating the venv
    $ command -v pip
    /usr/bin/pip
    # Modify your path to use the venv
    $ source jjb/bin/activate
    # Your path has been modified to use the venv
    (jjb)$ command -v pip
    <your venv managment dir>/jjb/bin/pip

The [Virtualenvwrapper][30] tool adds nice features on top of raw Virtualenv,
including management of venv directories.

    # Selection of most relevant commands
    ...
    # From any dir
    # Create basic venv (intall pip, wheel, dir structures) and modify your
    #     path to use the venv
    $ mkvirtualenv jjb
    # Your path has been modified to use the venv
    (jjb)$ command -v pip
    $WORKON_HOME/jjb/bin/pip
    # Without arguments, workon lists your venvs
    (jjb)$ workon
    <snip any other venvs>
    jjb

Whatever tools you choose to use to create a virtual environment, once you have
one the install process is the same. Note that our venv is active.

    # From your JBB repo clone
    # Install JJB's dependencies
...
example is provided at [`builder/jenkins.ini.example`][28].
    ...
    password=<your ODL Jenkins API token>
    url=https://jenkins.opendaylight.org/sandbox
    <snip>

To get your API token, [login to Jenkins][32], go to your user page (by
clicking on your username, for example), click "Configure" and then "Show API
Token".

## <a name="jjb_use_manual">Manual Method</a>

If you followed the [manual install method](#jjb_install_manual), you should
now activate your virtual environment to access the `jenkins-jobs` executable.
...
Once you've [verified your JJB jobs](#jjb_manual_test) produce valid XML
descriptions of Jenkins jobs you can push them to the Jenkins sandbox.

> _**Important Note:** When pushing with `jenkins-jobs` command, it will print
> out a message similar to the one below to inform you how many jobs JJB is pushing
...
> _**Failing to provide the final `<job-name>` param will result in all jobs
> being pushed!**_

    # Be not to push all jobs by omitting the final param! (ctrl+c to abort)
...
[32]: https://jenkins.opendaylight.org/releng/login "ODL Jenkins login"
